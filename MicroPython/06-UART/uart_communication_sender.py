...
Description: 
    ...
    This code acts as the SENDER and requires `uart_communication_receiver.py` (Receiver) running on another ESP32-C3-Zero.

Wiring:
    Tips: The wiring connections are crossed (TX to RX, RX to TX).
    
    [Sender Device]              [Receiver Device]
    ESP32-C3-Zero                 ESP32-C3-Zero
    Button (GPIO 6)               LED (GPIO 6)
    TX (GPIO 9)        ------>    RX (GPIO 2)
    RX (GPIO 8)        <------    TX (GPIO 3)
    GND                <------>   GND
...
- Tutorial [EN]: https://docs.waveshare.com/ESP32-MicroPython-Tutorials/UART-Communication#esp32-uart-communication
"""
...
# tx=9, rx=8: Specify transmit and receive pins
